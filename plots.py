...
def plot_one_box_kpt(x, im, color=None, label=None, line_thickness=3, kpt_label=False, kpts=None, steps=2, orig_shape=None, cid=0, size=256):
    ...
    if kpt_label:
        plot_skeleton_kpts(im, kpts, steps, orig_shape=orig_shape, idn=cid, fsize=size)
...
def plot_skeleton_kpts(im, kpts, steps, orig_shape=None, idn=0, fsize=256):
    builder = OscMessageBuilder(address=OSC_ADDRESS)
    builder.add_arg(int(idn))
    ...
    for kid in range(num_kpts):
        ...
        builder.add_arg(float(x_coord / fsize))
        builder.add_arg(float(y_coord / fsize))
