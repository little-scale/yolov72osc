...
def run(poseweights="yolov7-w6-pose.pt",source="football1.mp4",device='cpu',view_img=False,
    ...
    if (cap.isOpened() == False):   #check if videocapture not opened
        ...
        frame_width = 256  #set video frame width
        frame_height = frame_width #set video frame height

        
        vid_write_image = letterbox(cap.read()[1], (frame_width), stride=64, auto=True)[0] #init videowriter
        resize_height = frame_width
        resize_width = frame_height
                    ...
                    if len(output_data):  #check if no pose
                        ...
                        for det_index, (*xyxy, conf, cls) in enumerate(reversed(pose[:,:6])): #loop over poses for drawing on frame
                            ...
                            plot_one_box_kpt(xyxy, im0, label=label, color=colors(c, True), 
                                        ...
                                        orig_shape=im0.shape[:2], cid=det_index, size=frame_width)
